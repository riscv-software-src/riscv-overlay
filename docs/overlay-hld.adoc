...
The `-foverlay` flag enables overlay support in the compiler. Specifically,
this flag:
...
does not include class-static symbols).

Overlay functions cannot be inlined.

Overlay functions cannot make tail calls or be tail called.

Overlay functions must be 4 byte aligned.

Arithmetic cannot be done on overlay tokens.
