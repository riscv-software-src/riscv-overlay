...
the call to overlay function with a ‘JR’ instruction where x16 is the jump
...
The user on compilation time is defining the deep of the stack.
...
On RTOS based application, this pool is agnostic to the RTOS.
...
This register is being allocated only on RTOS based application.
...
LRU (least recently used) Like the search procedure, the eviction decision may
also accrue twice: i) When a new overlay function is called, ii) and after
returning to the RT-Engine while the return destination is an overlay function,
which was already evicted (can occur due to nested overlay function calls).
[[Load]]
=====	Load
