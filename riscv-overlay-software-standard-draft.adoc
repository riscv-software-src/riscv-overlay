...
1.  Compiler needs to generate a code for any related overlay usage, the sequence will lead to entering to the RT Engine were it will manage the process of loading, evicting, etc…
...
1.  Overlay symbols cannot be referenced with memory address, since they are not part of the physical memory. Therefor we should have a descriptor/token to describing the overlay symbol, for example for which group it is related? etc ...
2.  The linker will get all the necessary data from: objects, linker script and from a the linker input flags.
3.  Shall create an overlay section for each symbol addressed in the object (user define it on the code it self, by the attribute)
...
6.  There should be an *"_overlay area_"* to holds all the groups in the program. This area is not for execution, it is for linker to treat overlay function as normal functions: address allocation, optimization etc…
7.  Multi-group
+
...
a. The linker shall create a symbol table to hold all the groups, the table represent an index mapping for all overlay groups.
...
.  Debugger should hold a trace history (for call-stack) which includes the RT-Eng operations itself.
...
.  The debugger will be agnostics to the existence of and RTOS, this means a context switch can happen in an overlay operation and the debugger should hold a valid sequence. This can be achieved by "spec handshake" between RT-Eng data base and the debugger.
...
. We shall have debug information for overlay functions, that information should be symmetric if function is placed in several groups (*_multi group_*).
