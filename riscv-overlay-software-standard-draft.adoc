...
On RTOS based system, there are two needed hooks: 1) Critical section hooks and 2) Porting\bsp hooks
...
*_Porting/BSP hooks_*: if the engine needs to modify the kernel it should give a hook to the application for implementation.
Only the application knows which RTOS is been used and how to use it. The RT-Engine knows where to call this hook therefor the hook call will be
published with the RTOS ????
...
^|Xe|*Only on RTOS support*: Holds the task/thread stack pointer
|===
