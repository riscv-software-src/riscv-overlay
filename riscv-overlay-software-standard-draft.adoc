...
2.  Overlay group size ranges from 512B – 4K for both functions and constant-data.
3.  Group size will be decided in advance by the developer, and we will provided to at link-time.
4.  Overlay function or data should not be bigger then the maximum group size. 
...
2.  The heap should have range limitation to be in sync with the RT Engine and toolchain. The heap minimum size should not be the smaller then the maximum pre-defined overlay group size.
...
4.  We probably need to reserve a few core registers to be used only for the RT-Engine. Those registers will have a special purpose understood by the compiler, linker, debugger and RT-Engine. Moreover those registers forming a _RT-Eng-Debugger-handshake_ between compiler, RT code and debugger.
+
...
b. An Overlay data can be resident in more than one group.
